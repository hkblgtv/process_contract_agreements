...
def extract_data_with_docai(pdf_path, project_id, location, processor_id):
    """
    Sends a PDF to Google Cloud Document AI for data extraction.
    Returns the Document AI's response as a dictionary.
    """
    try:
        ...
        # projects/project-id/locations/location/processors/processor-id/processorVersions/processor-version-id
        # You must create new processors in the Cloud Console.
        ...
        print(f"Error during Document AI extraction for {pdf_path}: {e}")
...
def extract_data_from_text_with_llm(prompt):
    """
    Sends a text prompt to a Google LLM (Gemini) for data extraction.
    ...
    try:
        ...
        print(f"Error during LLM extraction from text: {e}")
...
def process_contract(contract_pdf_file, llm_prompt_fields, output_columns):
    ...
    try:
        ...
        if short_pdf_path:
            ...
            project_id = "dynamic-aurora-467007-q5" #"your-gcp-project-id"  # Replace with your GCP project ID
            location = "us"  # Replace with your Document AI processor location
            processor_id = "246e93e080df365" #"your-docai-processor-id"  # Replace with your Document AI processor ID
            processor_id = "c649821a479ca9b" #document ocr
            
            print(f"  - Sending {os.path.basename(short_pdf_path)} to Document AI for text extraction...")
            docai_text = extract_data_with_docai(short_pdf_path, project_id, location, processor_id)

            # Stop if Document AI returned an error
            if docai_text.startswith("Document AI Error:"):
                print(f"  - Halting processing for this file due to Document AI error.")
                extracted_data["Error"] = docai_text
                return extracted_data

            # Step 3: Construct LLM prompt dynamically with the extracted text
            llm_prompt = (
                "From the provided contract text, extract the following information:"
            )
            ...
            print(f"  - Sending extracted text to LLM for final extraction...")
            llm_response = extract_data_from_text_with_llm(llm_prompt)
...
def main():
    ...
    """
    output_csv_file = "extracted_contract_data_docai.csv"
